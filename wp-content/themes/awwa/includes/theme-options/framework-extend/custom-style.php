...
if ( ! function_exists( 'awwa_dynamic_styles' ) ) {
  function awwa_dynamic_styles() {

    // Typography
    $awwa_get_typography  = awwa_get_typography();
    ...
    // Logo
    $awwa_logo_top     = cs_get_option( 'awwa_logo_top' );
    $awwa_logo_bottom  = cs_get_option( 'awwa_logo_bottom' );
...
$layout_boxed = ( $bg_pattern === 'custom-pattern' ) ? 'background-image: url('. esc_url($custom_bg_pattern) .');' : 'background-image: url('. esc_url(AWWA_IMAGES) . '/patterns/'. $bg_pattern .'.png);';
?>
  .layout-boxed {
    <?php echo   wp_kses_data( $layout_boxed ); ?>
  }
...
if ($awwa_meta) {
  $transparent_header =  isset( $awwa_meta['transparency_header'] ) ? $awwa_meta['transparency_header'] : '' ;
  $transparent_menu_color = isset( $awwa_meta['transparent_menu_color'] ) ? $awwa_meta['transparent_menu_color'] : '' ;
  $transparent_menu_hover = isset( $awwa_meta['transparent_menu_hover_color'] ) ? $awwa_meta['transparent_menu_hover_color'] : '' ;
} else {
  $transparent_header = '';
...
<?php
  ...
  echo   wp_kses_data( $awwa_get_typography );
  $output = ob_get_clean();
  return $output;
 ...
 * Custom Font Family
 */
if ( ! function_exists( 'awwa_custom_font_load' ) ) {
  function awwa_custom_font_load() {
...
/* Custom Styles */
if( ! function_exists( 'awwa_custom_css' ) ) {
  function awwa_custom_css() {
    wp_enqueue_style('awwa-default-style', get_template_directory_uri() . '/style.css');
    $output  = awwa_custom_font_load();
    $output .= awwa_dynamic_styles();

    if( function_exists( 'awwa_compress_css_lines' ) ) {
      $custom_css = awwa_compress_css_lines( $output );
       wp_add_inline_style( 'awwa-default-style', $custom_css );
    }
    awwa_typography_fonts();
  }
  add_action( 'wp_enqueue_scripts', 'awwa_custom_css' );
}

/* Custom JS */
if( ! function_exists( 'awwa_custom_js' ) ) {
  function awwa_custom_js() {
    $output = cs_get_option( 'theme_custom_js' );
    if ($output) {
      wp_add_inline_script( 'jquery-migrate', $output );
    }
  }
  add_action( 'wp_enqueue_scripts', 'awwa_custom_js' );
}