...
if (function_exists('cs_framework_init')) {
  ...
  require_once( AWWA_CS_FRAMEWORK . '/theme-options.php' );
  require_once( AWWA_CS_FRAMEWORK . '/theme-customizer.php' );
  require_once( AWWA_CS_FRAMEWORK . '/theme-metabox.php' );
   ...
   */
  if( ! function_exists( 'awwa_custom_font_upload' ) ) {
    function awwa_custom_font_upload( $db_value ) {
    ...
    add_action( 'cs_typography_family', 'awwa_custom_font_upload' );
  }
 ...
 * Check Custom Font
 */
if ( ! function_exists( 'awwa_custom_upload_font' ) ) {
  function awwa_custom_upload_font( $font ) {
 ...
 */
if ( ! function_exists( 'awwa_registered_sidebars' ) ) {
  function awwa_registered_sidebars() {
 ...
 */
if ( ! function_exists( 'awwa_typography_fonts' ) ) {
  function awwa_typography_fonts() {
    ...
    if ( ! empty( $embed_fonts ) ) {
      ...
      wp_enqueue_style( 'awwa-typography-fonts', esc_url( add_query_arg( 'family', urlencode( implode( '|', $query_args ) ) . $subsets, '//fonts.googleapis.com/css2' ) ), array(), null );
    }
...
/* Typography */
if ( ! function_exists( 'awwa_get_typography' ) ) {
  function awwa_get_typography() {
    ...
    if ( ! empty( $typography ) ) {
      foreach ( $typography as $font ) {
        if ( ! empty( $font['selector'] ) ) {
          ...
          $weight  = ( $font['font']['variant'] != 'regular' ) ? awwa_esc_string( $font['font']['variant'] ) : 400;
          $style   = awwa_esc_number( $font['font']['variant'] );
          ...
          $output .= ( ! empty( $font['size'] ) ) ? 'font-size: '. awwa_check_px( $font['size'] ) .';' : '';
