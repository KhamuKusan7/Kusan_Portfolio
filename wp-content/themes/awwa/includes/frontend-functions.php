...
if( ! function_exists( 'awwa_excludecat' ) ) {
  function awwa_excludecat($query) {
  	if ( $query->is_home ) {
  ...
  add_filter('pre_get_posts', 'awwa_excludecat');
}
...
class AwwaExcerpt {
  ...
  public static function length($new_length = 55) {
    AwwaExcerpt::$length = $new_length;
    add_filter('excerpt_length', 'AwwaExcerpt::new_length');
    AwwaExcerpt::output();
  }

  // Tells WP the new length
  public static function new_length() {
    if( isset(AwwaExcerpt::$types[AwwaExcerpt::$length]) )
      return AwwaExcerpt::$types[AwwaExcerpt::$length];
    else
      return AwwaExcerpt::$length;
  }
...
if( ! function_exists( 'awwa_excerpt' ) ) {
  function awwa_excerpt($length = 55) {
    AwwaExcerpt::length($length);
  }
}

if ( ! function_exists( 'awwa_new_excerpt_more' ) ) {
  function awwa_new_excerpt_more( $more ) {
    return ' ';
  }
  add_filter('excerpt_more', 'awwa_new_excerpt_more');
}

/* Tag Cloud Widget - Remove Inline Font Size */
if( ! function_exists( 'awwa_tag_cloud' ) ) {
  function awwa_tag_cloud($tag_string){
    return preg_replace("/style='font-size:.+pt;'/", '', $tag_string);
  }
  add_filter('wp_generate_tag_cloud', 'awwa_tag_cloud', 10, 3);
}

/* Password Form */
if( ! function_exists( 'awwa_password_form' ) ) {
  function awwa_password_form( $output ) {
    $output = str_replace( 'type="submit"', 'type="submit" class=""', $output );
    return $output;
  }
  add_filter('the_password_form' , 'awwa_password_form');
}


/* Widget Layouts */
if ( ! function_exists( 'awwa_footer_widgets' ) ) {
  function awwa_footer_widgets() {
...
/* WP Link Pages */
if ( ! function_exists( 'awwa_wp_link_pages' ) ) {
  function awwa_wp_link_pages() {
    $defaults = array(
      'before'           => '<div class="wp-link-pages">' . esc_html__( 'Pages:', 'awwa' ),
      'after'            => '</div>',
      'link_before'      => '<span>',
      'link_after'       => '</span>',
...
/* Metas */
if ( ! function_exists( 'awwa_post_metas' ) ) {
  function awwa_post_metas() {
  ...
  <div class="bp-top-meta">
    ...
    <div class="bp-author">
      <?php
      printf(
        '<span>'. esc_html__('by','awwa') .' <a href="%1$s" rel="author">%2$s</a></span>',
...
/* Author Info */
if ( ! function_exists( 'awwa_author_info' ) ) {
  function awwa_author_info() {
    ...
    $author_text = $author_text ? $author_text : esc_html__( 'Author', 'awwa' );
...
=============================================== */
if ( ! function_exists( 'awwa_comment_modification' ) ) {
  function awwa_comment_modification($comment, $args, $depth) {
  ...
  <<?php echo esc_attr($tag); ?> <?php comment_class('item ' . $comment_class .' ' ); ?> id="comment-<?php comment_ID() ?>">
   ...
   <article>
    <div id="div-comment-<?php comment_ID() ?>" class="awwa-comment">
    <?php endif; ?>
    ...
    <div class="comment-main-area">
      ...
      <em class="comment-awaiting-moderation"><?php echo esc_html__( 'Your comment is awaiting moderation.', 'awwa' ); ?></em>
      <?php endif; ?>
      <div class="comment-content">
        ...
        <div class="comments-reply">
          <?php
            ...
            'reply_text' => '<span class="comment-reply-link icofont icofont-reply-all">'.esc_html__( 'Reply', 'awwa' ).'</span>',
            'before' => '',
            'class'  => '',
            'depth' => $depth,
...
if( ! function_exists( 'awwa_move_comment_field' ) ) {
  add_filter( 'comment_form_fields', 'awwa_move_comment_field' );
  function awwa_move_comment_field( $fields ) {
...
/* Title Area */
if ( ! function_exists( 'awwa_title_area' ) ) {
  function awwa_title_area() {
    ...
    if ($awwa_meta && (!is_archive() || is_woocommerce_shop())) {
      $custom_title = $awwa_meta['page_custom_title'];
      if ($custom_title) {
    ...
    if ( is_home() && is_front_page() ) {
      echo esc_html__( 'Blog', 'awwa' );
    } elseif ( is_home() && !is_front_page() ) {
      single_post_title();
    } elseif ( is_search() ) {
      printf( esc_html__( 'Search Results for %s', 'awwa' ), '<span>' . get_search_query() . '</span>' );
    } elseif ( is_category() || is_tax() ){
      single_cat_title();
    } elseif ( is_tag() ){
      single_tag_title( esc_html__('Posts Tagged: ', 'awwa') );
    } elseif ( is_archive() ){
      if ( is_day() ) {
        printf( wp_kses( __( 'Archive for <span>%s</span>', 'awwa' ), array( 'span' => array() ) ), get_the_date());
      } elseif ( is_month() ) {
        printf( wp_kses( __( 'Archive for <span>%s</span>', 'awwa' ), array( 'span' => array() )), get_the_date( 'F, Y' ));
      } elseif ( is_year() ) {
        printf( wp_kses( __( 'Archive for <span>%s</span>', 'awwa' ), array( 'span' => array() ) ), get_the_date( 'Y' ));
      } elseif ( is_author() ) {
        printf( wp_kses( __( 'Posts by: <span>%s</span>', 'awwa' ), array( 'span' => array() ) ), get_the_author_meta( 'display_name', $wp_query->post->post_author ));
      } elseif( is_woocommerce_shop() ) {
        esc_html_e( 'Shop', 'awwa' );
      } elseif( is_product() ) {
        esc_html_e( 'Shop Single', 'awwa' );
      } elseif ( is_post_type_archive() ) {
        post_type_archive_title();
      } else {
        esc_html_e( 'Archives', 'awwa' );
      }
    } elseif( ( class_exists( 'WooCommerce' ) ) && ( is_product() ) ) {
       the_title();
    } elseif( is_singular( 'event' ) ) {
     esc_html_e('Event Single', 'awwa');
    } elseif( is_singular( 'service' ) ) {
      the_title();
    } elseif( is_singular( 'team' ) ) {
      esc_html_e('Team Single', 'awwa');
    }  elseif( is_404() ) {
      esc_html_e('404 Error', 'awwa');
    } elseif( $custom_title ) {
...
// Pingback Head
function awwa_pingback_header() {
  if ( is_singular() && pings_open() ) {
    printf( '<link rel="pingback" href="%s">' . "\n", get_bloginfo( 'pingback_url' ) );
  }
}
add_action( 'wp_head', 'awwa_pingback_header' );


/**
 * blog pagination function.
 */
if ( ! function_exists( 'awwa_posts_navigation' ) ) :
  function awwa_posts_navigation() {
 ...
 */
if ( ! function_exists( 'awwa_single_pagination' ) ) :
  function awwa_single_pagination() {
    $newer_post = cs_get_option('newer_post');
    $newer_post = ( $newer_post ) ? $newer_post : esc_html__('Next Post', 'awwa');
    $older_post = cs_get_option('older_post');
    $older_post = ( $older_post ) ? $older_post : esc_html__('Previous Post', 'awwa');
    the_post_navigation(
      array(
        'next_text' => '<span class="meta-nav" aria-hidden="true">' . esc_html( 'Next Post', 'awwa' ) . '</span> ' .
          '<span class="screen-reader-text">' . esc_html( $newer_post ) . '</span> <br/>' .
          '<span class="post-title">%title</span>',
        'prev_text' => '<span class="meta-nav" aria-hidden="true">' . esc_html( 'Previous Post', 'awwa' ) . '</span> ' .
...
function awwa_unique_id( $prefix = '' ) {
  static $id_counter = 0;
...
if (!function_exists('awwa_header_function')) {
  function awwa_header_function(){
  ...
  $awwa_meta  = get_post_meta( $awwa_id, 'page_type_metabox', true );
  // Theme Layout Width
  $awwa_layout_width  = cs_get_option( 'theme_layout_width' );
  $theme_preloder  = cs_get_option( 'theme_preloder' );
  $awwa_layout_width_class = ( $awwa_layout_width === 'container' ) ? 'layout-boxed' : 'layout-full';
  // Header Style
  if ( $awwa_meta ) {
    $awwa_header_design  = $awwa_meta['select_header_design'];
    if ($awwa_header_design != 'theme') {
      $awwa_header_design = $awwa_header_design;
    } else {
  ...
  if (is_numeric($awwa_header_design)) {
  ...
  <body <?php body_class(); ?>>
     <?php wp_body_open(); ?>
  <div class="page-wrapper <?php echo esc_attr( $awwa_layout_width_class ); ?>"> <!-- #awwa-theme-wrapper -->
  <?php if( $theme_preloder ) {
   get_template_part( 'theme-layouts/header/preloder' );
    ...
    <header id="header" class="<?php echo esc_attr( $header_class ); ?>">
      <?php  if (is_numeric($awwa_header_design)) {
        $header_builder = new WP_Query(
          array(
            'post_type' => 'headerbuilder',
            'posts_per_page' => 1,
            'p' => $awwa_header_design,
            'orderby' => 'none',
            'order' => 'DESC'
          )
        );
    ...
    <?php
    // Title Area
    $awwa_need_title_bar = cs_get_option('need_title_bar');
    if ( !$awwa_need_title_bar ) {
