<?php
/*
 * All Back-End Helper Functions for Awwa Theme
 * Author & Copyright:wpoceans
 * URL: http://themeforest.net/user/wpoceans
 */

/* Validate px entered in field */
if( ! function_exists( 'awwa_check_px' ) ) {
  function awwa_check_px( $num ) {
    return ( is_numeric( $num ) ) ? $num . 'px' : $num;
  }
}

/* Escape Strings */
if( ! function_exists( 'awwa_esc_string' ) ) {
  function awwa_esc_string( $num ) {
    return preg_replace('/\D/', '', $num);
  }
}

/* Escape Numbers */
if( ! function_exists( 'awwa_esc_number' ) ) {
  function awwa_esc_number( $num ) {
...
if ( ! function_exists( 'awwa_tinymce_btns_font' ) ) {
  function awwa_tinymce_btns_font( $buttons ) {
    array_unshift( $buttons, 'fontselect' ); // Add Font Select
    array_unshift( $buttons, 'fontsizeselect' ); // Add Font Size Select
    return $buttons;
  }
  add_filter( 'mce_buttons_2', 'awwa_tinymce_btns_font' );
}

// Customize mce editor font sizes
if ( ! function_exists( 'awwa_tinymce_sizes' ) ) {
  function awwa_tinymce_sizes( $initArray ){
    $initArray['fontsize_formats'] = "9px 10px 12px 13px 14px 16px 18px 21px 24px 28px 32px 36px";
    return $initArray;
  }
  add_filter( 'tiny_mce_before_init', 'awwa_tinymce_sizes' );
}

// Customize mce editor font family
if ( ! function_exists( 'awwa_tinmymce_family' ) ) {
  function awwa_tinmymce_family( $initArray ) {
  ...
  add_filter( 'tiny_mce_before_init', 'awwa_tinmymce_family' );
}

/* HEX to RGB */
if( ! function_exists( 'awwa_hex2rgb' ) ) {
  function awwa_hex2rgb( $hexcolor, $opacity = 1 ) {
...
if ( is_admin() ) {
  ...
  if( ! function_exists( 'awwa_remove_rev_slider_meta_boxes' ) ) {
    function awwa_remove_rev_slider_meta_boxes() {
      remove_meta_box( 'awwa_metabox_revslider_0', 'page', 'normal' );
      remove_meta_box( 'awwa_metabox_revslider_0', 'post', 'normal' );
      remove_meta_box( 'awwa_metabox_revslider_0', 'service', 'normal' );
      remove_meta_box( 'awwa_metabox_revslider_0', 'testimonial', 'normal' );
      remove_meta_box( 'awwa_metabox_revslider_0', 'portfolio', 'normal' );
    }
    add_action( 'do_meta_boxes', 'awwa_remove_rev_slider_meta_boxes' );
  }

}

function awwa_project_add_icons_to_codestar_iconset() {

    $title  = esc_html__( 'Flat Icons', 'awwa' );
    $icons  = array(
        'flaticon-credit',
        'flaticon-job',
        'flaticon-team-work',
...
add_action( 'cs_add_icons', 'awwa_project_add_icons_to_codestar_iconset' );


function awwa_kingcomposer_fonts() {
...
add_action('init', 'awwa_kingcomposer_fonts');
