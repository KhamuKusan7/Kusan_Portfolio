 ...
 */
class awwa_footer_posts extends WP_Widget {
  ...
  public function __construct() {
    parent::__construct(
      '-footer-blog',
      AWWA_THEME_NAME_PLUGIN . esc_html__( ': Footer Posts', 'awwa' ),
      array(
        'classname'   => 'footer-post-widget',
        'description' => AWWA_THEME_NAME_PLUGIN . esc_html__( ' widget that displays footer posts.', 'awwa' )
      )
    );
  }
  ...
  public function form( $instance ) {
    ...
    $instance   = wp_parse_args( $instance, array(
      'title'    => esc_html__( 'Footer Posts', 'awwa' ),
      'ptypes'   => 'post',
      'limit'    => '3',
      'date'     => true,
      'category' => '',
    ...
    $ptypes_field = array(
      ...
      'type' => 'select',
      'options' => 'post_types',
      'default_option' => esc_html__( 'Select Post Type', 'awwa' ),
      'title' => esc_html__( 'Post Type :', 'awwa' ),
    );
    ...
    $limit_field = array(
      ...
      'type'  => 'text',
      'title' => esc_html__( 'Limit :', 'awwa' ),
      'help' => esc_html__( 'How many posts want to show?', 'awwa' ),
    );
    ...
    $date_field = array(
      ...
      'title' => esc_html__( 'Display Date :', 'awwa' ),
    );
    ...
    $category_field = array(
      ...
      'type'  => 'text',
      'title' => esc_html__( 'Category :', 'awwa' ),
      'help' => esc_html__( 'Enter category slugs with comma(,) for multiple items', 'awwa' ),
    );
    ...
    $order_field = array(
      ...
      'default_option' => esc_html__( 'Select Order', 'awwa' ),
      'title' => esc_html__( 'Order :', 'awwa' ),
    );
    ...
    $orderby_field = array(
      ...
      'type' => 'select',
      'options'   => array(
        'none' => esc_html__('None', 'awwa'),
        'ID' => esc_html__('ID', 'awwa'),
        'author' => esc_html__('Author', 'awwa'),
        'title' => esc_html__('Title', 'awwa'),
        'name' => esc_html__('Name', 'awwa'),
        'type' => esc_html__('Type', 'awwa'),
        'date' => esc_html__('Date', 'awwa'),
        'modified' => esc_html__('Modified', 'awwa'),
        'rand' => esc_html__('Random', 'awwa'),
      ),
      'default_option' => esc_html__( 'Select OrderBy', 'awwa' ),
      'title' => esc_html__( 'OrderBy :', 'awwa' ),
    );
  ...
  public function widget( $args, $instance ) {
    ...
    if ($awwa_rpw->have_posts()) : while ($awwa_rpw->have_posts()) : $awwa_rpw->the_post();
      ...
      if (!empty( $post_img )) {
        ...
        $post_img = AWWA_PLUGIN_IMGS.'/70X80.jpg';
      } ?>
...
add_action( 'widgets_init', function() { register_widget( "awwa_footer_posts" ); } );